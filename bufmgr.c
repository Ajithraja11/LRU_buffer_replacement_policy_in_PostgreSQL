			...
			bool *foundPtr)
{
	 ...
	 */
// usage count not necessary in LRU policy 
	buf->tag = newTag;
...
PinBuffer(BufferDesc *buf, BufferAccessStrategy strategy)
{
	...
	if (ref == NULL)
	{
		...
		for (;;)
		{
			...
			/* This block of code is not necessary because we don't need usage_count in LRU implementation
				if (strategy == NULL) 
			{
...
UnpinBuffer(BufferDesc *buf, bool fixOwner)
{
	...
	if (ref->refcount == 0)
	{
		...
		StrategyFreeBuffer(buf);//strategy free buffer is called once pin_count becomes zero
...
SyncOneBuffer(int buf_id, bool skip_recently_used, WritebackContext *wb_context)
{
	...
	//  usagecount is not necessary in LRU policy
